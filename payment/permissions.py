...
class IsPaymentPending(BasePermission):
    ...
    message = "Updating or deleting completed payment is not allowed."
...
class IsPaymentForOrderNotCompleted(BasePermission):
    message = "Creating a checkout session for completed payment is not allowed."
...
class DoesOrderHaveAddress(BasePermission):
    message ="Creating a checkout session without having a shipping " \
    "and billing address is not allowed."
...
class IsOrderPendingWhenCheckout(BasePermission):
    ...
    message = "Updating closed order is not allowed."
